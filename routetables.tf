...
resource "oci_core_route_table" "PrivateRouteTable" {
  ...
  dynamic "route_rules" {
    for_each = toset(var.nat_gateway_present != true ? [] : [0])
    content {
  ...
  dynamic "route_rules" {
    for_each = toset(var.service_gateway_present != true ? [] : [0])
    content {
        network_entity_id = oci_core_service_gateway.PrivateServiceGateway[0].id
...
resource "oci_core_route_table" "PublicRouteTable" {
  ...
  dynamic "route_rules" {
    for_each = toset(var.nat_gateway_present != true ? [] : [0])
    content {
        network_entity_id = oci_core_internet_gateway.INETGateway[0].id
