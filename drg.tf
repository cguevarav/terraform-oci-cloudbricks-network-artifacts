...
resource "oci_core_drg_attachment" "DynamicRoutingGatewayHubAttachment" {
  count      = var.is_spoke ? 1 : 0
  ...
  drg_id     = oci_core_drg.DynamicRoutingGateway[0].id
...
# resource "oci_core_drg_attachment" "DynamicRoutingGatewaySpokeAttachment" {
#   count      = var.is_spoke ? 0 : 1
#   depends_on = [oci_core_drg.DynamicRoutingGateway]
#   // drg_id     = length(local.hub_drg) > 0 ? local.hub_drg[0].id : oci_core_drg.DynamicRoutingGateway[0].id
#   // drg_id     = oci_core_drg.DynamicRoutingGateway[0].id
#   drg_id     = local.hub_drg.id
